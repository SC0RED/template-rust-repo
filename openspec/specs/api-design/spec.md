...
All API endpoints MUST validate request input using schema validation (Zod in TypeScript, Pydantic in Python, serde + a validator in Rust) at the handler/route level. Validation MUST occur before any business logic executes. Invalid requests MUST return immediately with a descriptive error — they MUST NOT propagate into service or domain layers.
