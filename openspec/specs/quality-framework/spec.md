...
4. **No Unsafe Constructs** — Language-specific unsafe constructs MUST be forbidden (`any` in TypeScript; bare `except` in Python; `unsafe`, `unwrap()`/`expect()`/`panic!` in non-test code, and `as` casts in Rust).
...
|---------|------------|
| Types | PascalCase |
| Functions | snake_case; verb-prefixed except idiomatic accessors named for the field |
| Constants | SCREAMING_SNAKE_CASE |
| Variables | snake_case |
| Files | snake_case; generic names (`utils`, `helpers`, `common`) forbidden |

Case conventions are enforced mechanically by the compiler and clippy. Functions
SHOULD start with a verb, except language-idiomatic accessors (Rust getters are
named for the field — `name()`, not `get_name()`). The naming scripts enforce the
checkable rules: no generic file names, no forbidden abbreviations, no skip comments.

#### Scenario: Generic File Name Rejected
- **GIVEN** A developer adds `src/utils.rs`
- **WHEN** `scripts/check_naming.sh` runs
- **THEN** The script MUST exit non-zero and name the offending file

#### Scenario: Forbidden Abbreviation Rejected
- **GIVEN** A developer introduces an identifier `ctx`
- **WHEN** `scripts/check_abbreviations.sh` runs
- **THEN** The script MUST exit non-zero and report the abbreviation

### Requirement: Forbidden Abbreviations

The template MUST enforce full words over abbreviations via automated tooling. The Rust list is intentionally conservative — it excludes tokens that collide with the language (`cfg`, `impl`, `err`/`Err`, `env`). At minimum, these MUST be forbidden: msg, req, resp, ctx, mgr, usr, pwd, tmp, temp, btn. Extend per project.
...
The template MUST enforce a minimum line-coverage gate (90% for Rust, via `cargo llvm-cov --fail-under-lines 90`, excluding the binary entry point). Tests MUST focus on behavior, not implementation.

#### Scenario: Coverage Below Threshold
- **GIVEN** The test suite achieves 85% line coverage
- **WHEN** `make test` runs
