...
#
# Usage:
#   ./scripts/git_push_as_bot.sh [remote] [refspec]
#   ./scripts/git_push_as_bot.sh                        # push current branch to origin
#   ./scripts/git_push_as_bot.sh origin feature/my-branch
#   ./scripts/git_push_as_bot.sh origin HEAD:refs/heads/feature/my-branch
#
# Prerequisites:
...
if [ -f /tmp/gh_app_token ]; then
    ...
    if [ "$HTTP_CODE" != "200" ]; then
        ...
        TOKEN=$("${SCRIPT_DIR}/generate_github_app_token.sh")
    fi
else
